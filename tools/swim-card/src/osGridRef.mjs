...
// .northing), not lat/long/dist. See beachQuality.mjs for why this exists.
//
...
// worked example (see expo/src/services/OsGridRef.test.ts, the app's
// identical, independently-written copy of this module) — the Helmert step
// isn't independently verified from here (no WGS84-in/OSGB36-out ground
// truth was available to fetch), but WGS84 and OSGB36 already agree to
// within ~100m across Great Britain, which the bounding-box margin this
// feeds into comfortably absorbs.
//
// Deliberately duplicated rather than shared with expo/src/services/
// OsGridRef.ts — this repo has no shared package between any of its
// clients (see docs/architecture.md), and this CLI is outside that
// four-client system entirely.

const WGS84 = { a: 6378137, b: 6356752.314245, f: 1 / 298.257223563 };
const AIRY_1830 = { a: 6377563.396, b: 6356256.909, f: 1 / 299.3249646 };
...
function toArcsecRadians(arcseconds) {
...
// lat/long for a beach, never an elevation, and the error that introduces
// is well under the bounding-box margin this feeds into.
function toCartesian(latitude, longitude, ellipsoid) {
...
function toGeodetic(cartesian, ellipsoid) {
...
function applyHelmertTransform(cartesian) {
  const { x, y, z } = cartesian;
...
function osgb36ToGridRef(latitude, longitude) {
  const phi = toRadians(latitude);
  const lambda = toRadians(longitude);
  const { a, b } = AIRY_1830;
...
export function wgs84ToOsGridRef(latitude, longitude) {
